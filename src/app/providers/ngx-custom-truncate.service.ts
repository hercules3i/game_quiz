...
export class NgxCustomTruncateService {

  constructor() { }

  /** check the word cut */
  checkWordCut(numberIn: number, text: string): string {
    let i = 0;
    for (i = numberIn; i < text.length; i++) {
      if (text[i] === ' ') {break;}
    }
  ...
  findHashtag(text) {
    ...
    text = text.replace(/(^|\s)(#[\p{Pc}\p{N}\p{L}\u200cÀ-ÖØ-öø-ʸ(_)]+)/ugmi,
    (match) => `<span class="hashtag" style="color:#1b95e0">${match}</span>`);
    return text;
  }

  /** check conditions and return text with them */
  applyCondition(text: string, numberIn: number, completeWord: boolean = false, hashtag: boolean = false): string {
    let manipulatedText = '';
    if (completeWord && hashtag) {
      const temp = this.checkWordCut(numberIn, text);
      manipulatedText = this.findHashtag(temp);
    }
    else if (completeWord) {
      manipulatedText = this.checkWordCut(numberIn, text);
    }
    else if (hashtag) {
      const temp = text.substring(0, numberIn);
      manipulatedText = this.findHashtag(temp);
    }
    else {
      manipulatedText = text.substring(0, numberIn);
    }
  ...
  highlight(content: any, highlightCondition: string , highlightQuery: any) {
    ...
    if (highlightCondition === 'exactly') {
      for (const q of highlightQuery) {
        let pattern = '';
        //به صورت آبجکت باشند highlightList اگر کلمات داده شده در
        if (q.hasOwnProperty('name')) {
          pattern = '(^|(?<!\\p{L}))(' + q.name + ')(?!(\\p{L}))';
          content = content.replace(new RegExp(pattern, 'gmu'), match => `<span style="background:${q.color}">${match}</span>`);
        }
        //به صورت رشته باشند highlightList اگر کلمات داده شده در
        else {
          pattern = '(^|(?<!\\p{L}))(' + q + ')(?!(\\p{L}))';
          content = content.replace(new RegExp(pattern, 'gmu'), match => `<span style="background:yellow">${match}</span>`);
        }
      }
      setTimeout(() => {
    ...
    //هر مشابهی از کلمات در متن پیدا شده و رنگ می شوند
      ...
      for (const q of highlightQuery) {
        ...
        //به صورت آبجکت باشند highlightList اگر کلمات داده شده در
        if (q.hasOwnProperty('name')) {
          const words = content.match(new RegExp(q.name, 'gmi'));
          if (words)
        ...
        //به صورت رشته باشند highlightList اگر کلمات داده شده در
          ...
          const words = content.match(new RegExp(q, 'gmi'));
          if (words)
            {for (const word of words) {
